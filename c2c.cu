...
int main(){
    ...
    int r_numel = std::accumulate(r_shape.begin(), r_shape.end(), 1, std::multiplies<int>());
    std::array<long long int, 4> c_shape {1, 4, 28, 54};
    int c_numel = std::accumulate(c_shape.begin(), c_shape.end(), 1, std::multiplies<int>());
