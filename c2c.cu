...
int main() {
  ...
  cufftXtMakePlanMany(a.get(), 2, r_shape.data() + 2, nullptr, 1, 1, CUDA_R_32F,
                      nullptr, 1, 1, CUDA_C_32F, 4, &work_size, CUDA_C_32F);
  ...
  for (int i = 0; i < 10; i++) {
    ...
    cufftXtExec(a.get(), thrust::raw_pointer_cast(dx.data()),
